from fastapi import APIRouter, Depends, File, Form, HTTPException, Request, UploadFile, Body, status
from fastapi.responses import StreamingResponse
from fastapi.encoders import jsonable_encoder

from pydantic import ValidationError

from fastapi_core.courses.schemas import Course, CourseCreate, Category, CategoryCreate, CourseUpdate, DifficultTypeChoices
...
async def get_courses(rating: str = None, 
                      study_hours: str = None, 
                      language: str = None, 
                      price: str = None, 
                      course_repo = Depends(get_repository(CourseAPIRepository))) -> List[Course]:
...
async def patch_course(course_id: int, 
...
async def upload_course_video(course_id: int, video: Optional[UploadFile] = File(...),course_repo = Depends(get_repository(CourseAPIRepository))):

    course = await course_repo.async_get_course(course_id=course_id)

    path = f"{MEDIA_PATH}/courses/{course_id}"
    out_video_name = await upload_file(path=path, filename=video.filename, in_file = video)
...
async def upload_course_logo(course_id: int, file: Optional[UploadFile] = File(...),course_repo = Depends(get_repository(CourseAPIRepository))):
...
async def get_course_video(course_id: int,
    ...
    file, status_code, content_length, headers = await course_repo.open_file(request, course.video)
...
@courses_router.post("/courses/{course_id}/favorite/", dependencies=[Depends(JWTBearer())])
async def favorite_course(course_id: int, current_user = Depends(get_current_user), course_repo = Depends(get_repository(CourseAPIRepository))):
    course = await course_repo.async_get_course(course_id = course_id)
...
async def get_favorites(current_user = Depends(get_current_user), course_repo = Depends(get_repository(CourseAPIRepository))):
