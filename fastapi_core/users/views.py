...
from fastapi_core.users.schemas import (
    UserRegister,
    UserLogin,
    User,
    UserUpdate,
...
@users_router.get("/users/personal/", tags=["users"], response_model=User)
async def user_current_detail(current_user=Depends(get_current_user)):
    return current_user


@users_router.patch("/users/personal/", response_model=User)
async def user_current_update(
...
@users_router.post("/users/login/", tags=["users"], response_model=LoginResponse)
async def user_login(
    user_login: UserLogin, response: Response, db: AsyncSession = Depends(async_get_db)
):
...
@users_router.patch("/users/change_password/", tags=["users"], response_model=User)
async def change_password(
    user_change_password_data: UserChangePassword,
    current_user=Depends(get_current_user),
):
...
@users_router.post("/users/upload_image/", response_model=User)
async def upload_image(
    image: UploadFile = File(...), current_user=Depends(get_current_user)
):

    path = f"{MEDIA_PATH}/users/{current_user.id}"
    out_image_name = await upload_file(
        path=path, filename=image.filename, in_file=image
    )
...
async def user_register(
    user_in: UserRegister, admin_repo=Depends(get_repository(AdminAPIRepository))
):
